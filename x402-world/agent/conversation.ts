...
import { ActionCtx, internalQuery } from '../convex/_generated/server';
...
export const queryPromptData = internalQuery({
  ...
  handler: async (ctx: any, args: any) => {
    ...
    const otherPlayer = world.players.find((p: any) => p.id === args.otherPlayerId);
    if (!otherPlayer) {
      throw new Error(`Player ${args.otherPlayerId} not found`);
    }
    const otherPlayerDescription = await ctx.db
      .query('playerDescriptions')
      .withIndex('worldId', (q: any) => q.eq('worldId', args.worldId).eq('playerId', args.otherPlayerId))
      .first();
    ...
    const agentDescription = await ctx.db
      ...
      .withIndex('worldId', (q: any) => q.eq('worldId', args.worldId).eq('agentId', agent.id))
      .first();
    if (!agentDescription) {
      throw new Error(`Agent description for ${agent.id} not found`);
    }
    const otherAgent = world.agents.find((a: any) => a.playerId === args.otherPlayerId);
    let otherAgentDescription: any;
    if (otherAgent) {
      otherAgentDescription = await ctx.db
        .query('agentDescriptions')
        .withIndex('worldId', (q: any) => q.eq('worldId', args.worldId).eq('agentId', otherAgent.id))
        .first();
