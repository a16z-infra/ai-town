...
import { MutationCtx, internalMutation, internalQuery } from '../convex/_generated/server';
...
export class Agent {
  ...
  tick(game: Game, now: number) {
    ...
    if (!conversation && !doingActivity && (!player.pathfinding || !recentlyAttemptedInvite)) {
      // @ts-expect-error - Legacy operation name, needs adaptation
      this.startOperation(game, now, 'agentDoSomething' as any, {
    ...
    if (this.toRemember) {
      ...
      // @ts-expect-error - Legacy operation name, needs adaptation
      this.startOperation(game, now, 'agentRememberConversation' as any, {
        worldId: game.worldId,
      ...
      if (member.status.kind === 'participating') {
        ...
        if (!conversation.lastMessage) {
          ...
          if (isInitiator || awkwardDeadline < now) {
            ...
            // @ts-expect-error - Legacy operation name, needs adaptation
            this.startOperation(game, now, 'agentGenerateMessage' as any, {
              worldId: game.worldId,
              playerId: player.id,
              agentId: this.id,
  ...
  startOperation<Name extends keyof AgentOperations>(
    ...
    console.log(`Agent ${this.id} starting operation ${name} (${operationIdStr})`);
    game.scheduleOperation(String(name), { operationId: operationIdStr, ...args } as any);
...
// NOTE: These operations are legacy from ai-town and may not exist in x402-world
// They need to be implemented in convex/x402World/agentOperations.ts or adapted
type AgentOperations = typeof internal.x402World.agentOperations;

export async function runAgentOperation(ctx: MutationCtx, operation: string, args: any) {
  // TODO: Implement these operations in x402-world or adapt to use existing operations
  // For now, this will fail at runtime if the operations don't exist
  let reference: any;
  switch (operation) {
    case 'agentRememberConversation':
      // @ts-expect-error - Legacy operation, needs implementation
      reference = internal.x402World.agentOperations.agentRememberConversation;
      break;
    case 'agentGenerateMessage':
      // @ts-expect-error - Legacy operation, needs implementation
      reference = internal.x402World.agentOperations.agentGenerateMessage;
      break;
    case 'agentDoSomething':
      // @ts-expect-error - Legacy operation, needs implementation
      reference = internal.x402World.agentOperations.agentDoSomething;
      break;
    default:
...
export const agentSendMessage = internalMutation({
  ...
  handler: async (ctx: any, args: any) => {
    // TODO: Add messages table to schema or use alternative storage
...
export const findConversationCandidate = internalQuery({
  ...
  handler: async (ctx, { now, worldId, player, otherFreePlayers }) => {
    ...
    for (const otherPlayer of otherFreePlayers) {
      // Find the latest interaction between these agents
      // Using agentInteractions table instead of participatedTogether
      const lastInteraction = await ctx.db
        .query('agentInteractions')
        .withIndex('edge', (q: any) =>
          q.eq('worldId', worldId).eq('agent1', player.id).eq('agent2', otherPlayer.id),
        )
        .order('desc')
        .first();
      if (lastInteraction?.ended) {
        if (now < lastInteraction.ended + PLAYER_CONVERSATION_COOLDOWN) {
          continue;
        }
      }
