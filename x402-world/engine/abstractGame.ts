...
import { ActionCtx, DatabaseReader, MutationCtx, internalQuery } from '../convex/_generated/server';
import { engine } from '../convex/engine/schema';
    ...
    while (numTicks < this.maxTicksPerStep) {
      ...
      for (const input of tickInputs) {
        ...
        try {
          ...
          console.error(`Input ${input._id} failed: ${error.message}`);
          returnValue = { kind: 'error' as const, message: error.message };
        }
