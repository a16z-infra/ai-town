...
import { internalMutation, query, action, httpAction, internalAction, internalQuery } from '../_generated/server';
...
export function replicateAvailable(): boolean {
    console.log("replicate token", process.env.REPLICATE_API_TOKEN, !!process.env.REPLICATE_API_TOKEN)
    return (
...
async function backgroundMusicGenHandler(prediction: Prediction, ctx: ActionCtx) {
    ...
    return await ctx.runMutation(internal.music.createMusic, { type: "background", storageId: storageId })
}
...
export const enqueueBackgroundMusicGeneration = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        const webhookId: Id<'replicate_webhooks'> = await ctx.runMutation(internal.lib.replicate.createEntry, { externalId: id, handler: 'BackgroundMusicGen', input: metadata })
        return webhookId
    },
})
...
export const getEntry = internalQuery({
    ...
    handler: async (ctx, { externalId, ...args }) => {
        const webhook = await ctx.db.query('replicate_webhooks')
            .filter((entry) => entry.eq(entry.field("externalId"), externalId)).first()
        return webhook;
    },
});
