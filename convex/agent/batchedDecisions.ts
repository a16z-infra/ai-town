...
import { ActionCtx, DatabaseReader, internalAction, query } from '../_generated/server';
...
export const batchedAgentRunOnce = internalAction({
  args: {
    worldId: v.id('worlds'),
  },
  handler: async (ctx, args) => {
    const decisions = await ctx.runQuery(api.agent.batchedDecisions.batchedAgentDecide, args);
    const runs = [];
    for (const { agentId, decision, playerId } of decisions) {
      runs.push(runDecision(ctx, agentId, playerId, decision));
    }
...
async function runDecision(
  ...
  switch (decision.kind) {
    ...
    case 'waitForInviteTimeout': {
      console.log(`Would sleep for ${decision.deadline - Date.now()}ms`);
      return;
    }
    case 'moveTo': {
    ...
    case 'participateInConversation':
      {
        ...
        switch (conversationDecision.kind) {
          ...
          case 'waitForCooldown': {
            console.log(`Would sleep for ${conversationDecision.deadline - Date.now()}ms`);
            return;
          }
          case 'sendMessage': {
